			...
			for (const type of TYPES) {
				...
				*/
				if (!this.#ramNet.assign(job)) {
					...
					throw `WARN: Insufficient RAM to assign ${job.type}: ${job.batch}.`;
				}
	...
	async run() {
		...
		while (true) {
			...
			while (!dataPort.empty()) {
				...
				if ((Object.prototype.toString.call(data) == "[object String]") && data.startsWith("weaken2")) {
...
export async function main(ns) {
  ...
  if(ns.args[0] == "--suicide") {
	...
	*/
	// if (ns.isRunning("/batcher/logHelper.js", "home")) ns.kill("/batcher/logHelper.js", "home");
	// const logPort = ns.exec("/batcher/logHelper.js", "home");
